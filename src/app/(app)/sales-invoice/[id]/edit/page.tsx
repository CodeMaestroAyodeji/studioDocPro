  ...
  if (!invoice) {
    return (
      <div className="flex flex-1 flex-col">
        ...
        <main className="flex-1 p-6 text-center"><p>Loading invoice for editing...</p></main>
      </div>
    );
  }
    ...
    <div className="flex flex-1 flex-col">
      ...
      <main className="flex-1 p-4 sm:px-6 sm:py-0 space-y-4">
        <div className="flex justify-end sticky top-[57px] sm:top-0 z-10 py-2 bg-background no-print gap-2">
          <Button variant="outline" onClick={() => router.push(`/sales-invoice/${invoiceId}`)}>Cancel</Button>
              ...
              <header className="grid grid-cols-2 gap-8 mb-8">
                ...
                <div className="text-right">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-sm">
                    <span className="font-semibold">Invoice #</span><span>{form.watch('invoiceNumber')}</span>
                  </div>
                </div>
              </header>
              ...
              <section className="grid md:grid-cols-3 gap-x-8 mb-8">
                <FormField control={form.control} name="billTo" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Client Name&#10;Client Address&#10;Client Contact" {...field} className="min-h-[80px]" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                  ...
                  <TableBody>
                    {fields.map((field, index) => (
                      <TableRow key={field.id}>
                        ...
                        <TableCell className="text-right font-medium">{formatCurrency((watchedItems[index]?.quantity || 0) * (watchedItems[index]?.unitPrice || 0))}</TableCell>
                        <TableCell><Button type="button" variant="ghost" size="icon" onClick={() => remove(index)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                <div className="mt-4"><Button type="button" variant="outline" onClick={() => append({ id: crypto.randomUUID(), description: '', quantity: 1, unitPrice: 0 })}><PlusCircle className="mr-2 h-4 w-4" /> Add Item</Button></div>
              </section>

               <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <FormField control={form.control} name="notes" render={({ field }) => (
                        <FormItem><FormLabel>Notes / Terms</FormLabel><FormControl><Textarea {...field} /></FormControl></FormItem>
                    )} />
